##blrm prior calibration results extractions stage 1

#start at 4
#all scens
load("JOINT_TITE_BLRM_priorcal1.RData")
    ...
    for (mu_alphaT_Z_i in 1:length(mu_alphaT_Z_priors)){
      for (mu_betaT_Z_i in 1:length(mu_betaT_Z_priors)){
        ...
        for(scen.index in 1:length(prior_scen_list)){
          ...
          # browser()
          res_list<- get(paste(c("JOINTTITEBLRM.prior_alpha_mu_Z",mu_alphaT_Z_i,"_beta_mu_Z",mu_betaT_Z_i,"_alpha_mu_W",mu_alphaT_W_i,"_beta_mu_W",mu_betaT_W_i,"_scen",tox,"_start4"),collapse=""))
          ...
          if(MTD=="UNSAFE"){
            ...
            result_array[scen.index,mu_alphaT_W_i,mu_betaT_W_i,mu_alphaT_Z_i,mu_betaT_Z_i]<-100*sum(safety)/nsims
          }else if(MTD=="TOOSAFE"){
            stop_list<-(lapply(res_list,'[[','stop.code'))
            stop_mat<-do.call(rbind,stop_list)
            #total of safety
            safety1<-(stop_mat[,7]>0)
            result_array[scen.index,mu_alphaT_W_i,mu_betaT_W_i,mu_alphaT_Z_i,mu_betaT_Z_i]<-100*sum(safety1,na.rm=T)/nsims
            
          }else{
            result_array[scen.index,mu_alphaT_W_i,mu_betaT_W_i,mu_alphaT_Z_i,mu_betaT_Z_i]<-100*sum(dose_rec_vector==MTD,na.rm=T)/nsims
          }               
...
par_choices<-c(mu_alphaT_W_priors[geomean_choice[1]],# mean alpha
               mu_betaT_W_priors[geomean_choice[2]],# mean beta
               mu_alphaT_Z_priors[geomean_choice[3]],# prec alpha
               mu_betaT_Z_priors[geomean_choice[4]])# prec beta) #skeleton index
