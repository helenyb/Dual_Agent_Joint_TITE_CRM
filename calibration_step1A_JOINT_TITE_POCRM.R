#1st round of prior cals, changing prior 6 skeletons and mu
  ...
  for (mu_betaT_i in 1:length(mu_betaT_priors)){
    ...
    for(skeleton_i in 1:6){
      ...
      for(scen.index in 1:length(prior_scen_list)){
        tox<-scen.index
        # browser()
        assign(paste(c("JOINTTITEPOCRM.prior_sk",skeleton_i,"_alpha",mu_alphaT_i,"_beta",mu_betaT_i,"_scen",tox,"_start",starting.dose),collapse=""),
               foreach(i=1:nsims, combine = list) %dopar% { #full (clusters)
                 ...
                 JOINT.TITE.POCRM.priorcal(seed=i,tru.E.pars=eff_scen1.1pars,tru.T.pars=get(paste(c("prior_scen",tox,".pars"),collapse="")),tru.corET=-0.5,
                                     ...
                                     wmT=wmT,prior.o.T=prior.probsT,default.order=c(1:10),start.dose=starting.dose)
               })#for assign
        
        
        save.image(paste(c("JOINT_TITE_POCRM_priorcal_st4.RData"),collapse=""))
      }#for scen
    }#for skeleton
  } #for beta
} #for alpha
