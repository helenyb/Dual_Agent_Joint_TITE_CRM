##For utility based on POCRM:

#load:
#boin st22
load("JOINT_TITE_BOIN_sims.RData")


methods_vec<-c("JOINTTITEBOIN")
#margin from max utility
...
dose_rec_frame<-function(method,eff_sc,tox_sc,eff_pattern){
  res_list<-get(paste(c(method,".eff",eff_sc,".",eff.pattern,"_tox",tox_sc,"st",starting.dose),collapse=""))
...
#dose_rec_frame(prec.rule = "FALSE",stop.num = 9,eff_sc=1,tox_sc=1)
...
for(method in methods_vec){
  #for(method in c("JOINTTITEPOCRM_p","JOINTTITEBLRM")){
  assign(paste(c("correct_acc_mt_",method),collapse=""),
         correct_acceptable_good_func(method=method,eff_sc_vec=c(1:8),tox_sc_vec=c(1:6),eff_pattern_vec=1,w1=0.33,w2=1.09,utility.margin = 0.1)
  )
...
# 
# for(method in methods_vec){
#   rec.all.mat<-matrix(nrow=22,ncol=42)
#   top.left.cell<-c(1,1)
#   for(tox.i in 1:6){
#     top.left.cell[1]<-4*tox.i-3
#     for(eff.i in 1:6){
#       top.left.cell[2]<-7*eff.i-6
#       rec.all.mat[top.left.cell[1]:(top.left.cell[1]+1),top.left.cell[2]:(top.left.cell[2]+5)]<-dose_rec_matrix(method=method,eff_sc= eff.i,tox_sc= tox.i,eff_pattern= 1)
#     }
#   }
#   assign(paste(c("rec.all.mat",method),collapse=""), rec.all.mat
#   )
#   rm(rec.all.mat)
# }

#View(round(rec.all.matJOINTTITEBLRM,1))


########
# 
# for(method in methods_vec){
#   ass.all.mat<-matrix(nrow=22,ncol=42)
#   top.left.cell<-c(1,1)
#   for(tox.i in 1:6){
#     top.left.cell[1]<-4*tox.i-3
#     for(eff.i in 1:6){
#       top.left.cell[2]<-7*eff.i-6
#       ass.all.mat[top.left.cell[1]:(top.left.cell[1]+1),top.left.cell[2]:(top.left.cell[2]+4)]<-dose_assi_matrix(method=method,eff_sc= eff.i,tox_sc= tox.i,eff_pattern= 1)
#     }
#   }
#   assign(paste(c("ass.all.mat",method),collapse=""), ass.all.mat
#   )
#   rm(ass.all.mat)
# }
#View(round(ass.all.matJOINTTITEBLRM,1))
...
SampleSize_func_all<-function(method,eff_sc_vec,tox_sc_vec,eff_pattern){
  ...
  for(eff in eff_sc_vec){
    for(tox in tox_sc_vec){
      ...
      res_list<-get(paste(c(method,".eff",eff,".",eff.pattern,"_tox",tox,"st",starting.dose),collapse=""))
...
for(method in methods_vec){
  ...
  assign(paste(c("SampleSize_mt_",method),collapse=""),
         SampleSize_func_all(method=method,eff_sc_vec=c(1:8),tox_sc_vec=c(1:6),eff_pattern=1)
  )
...
unsafe_list_func(eff_sc_vec = c(1:8), tox_sc_vec = c(1:6))
...
for(method in methods_vec){
  ...
  assign(paste(c("Unsafe_assigment_mt_",method),collapse=""),
         unsafe_func(method=method,eff_sc_vec=c(1:8),tox_sc_vec=c(1:6),eff_pattern_vec=1,co_size=3,ndoses=10)
  )
...
correct_acc_mt_JOINTTITEBOIN<-correct_acc_mt_JOINTTITEBOIN_st4

SampleSize_mt_JOINTTITEBOIN<-SampleSize_mt_JOINTTITEBOIN_st4

Unsafe_assigment_mt_JOINTTITEBOIN<-Unsafe_assigment_mt_JOINTTITEBOIN_st4
