#Joint TITE-BLRM prior calibration stage 2
...
mu_alphaT_Z_priors<-c(-5.5,-5)
mu_alphaT_W_priors<-c(-6)
tau_alphaT_priors<-c(1,2,4)
mu_betaT_Z_priors<-c(1,1.5)
mu_betaT_W_priors<-c(0)
        ...
        for (tau_alphaT_i in 1:length(tau_alphaT_priors)){
          for (tau_betaT_i in 1:length(tau_betaT_priors)){
            ...
            for(scen.index in 1:length(prior_scen_list)){
              tox<-scen.index
              # browser()
              assign(paste(c("JOINTTITEBLRM.prior_","alpha_tau",tau_alphaT_i,"_beta_tau",tau_betaT_i,
              ...
              save.image(paste(c("JOINT_TITE_BLRM_priorcal2.RData"),collapse=""))
            }#for scen
