...
JOINT.TITE.BOIN_CRMutility<-function(seed,tru.E.pars,tru.T.pars,tru.corET,co_size,ncohorts ,target,targetE,
                             ...
                             upper_tox_lim=0.35,pause=T,a.stop.bound, w1, w2, upper.tox){
  set.seed(seed)
  stop_vec<-rep(0,7)
  ...
  while(stop==0){
    ...
    if(initial==0){
      ...
      prob_over_eff<-prob_under_tox<-P_UB<-pi_T_hat<-pi_E_hat<-UTILITY<-c()
      ...
      for(dose_i in 1:ndoses){
        ...
        if(sum(patdoses==dose_i)>0){
          ...
          UTILITY[dose_i]<- pi_E_hat[dose_i] -
            w1*pi_T_hat[dose_i] -
            w2*pi_T_hat[dose_i]*as.numeric( pi_T_hat[dose_i]>upper.tox)
     ...
     #NEW:choose admissible dose with max utility
       ...
       # nextdose<-allowable_next_adm[which(P_UB[allowable_next_adm]==max(P_UB[allowable_next_adm]))]
         nextdose<-allowable_next_adm[which(UTILITY[allowable_next_adm]==max(UTILITY[allowable_next_adm]))]
    ...
    if(hard.safety==T){
      ...
      if(!is.na(nextdose)){
        if(excluded[nextdose]>0){#if the next dose is in the exclusion zone
          ...
          included.UTILITY[-allowable_next_adm]<--100
    ...
    if(nrow(current.patient.data)==(ncohorts*co_size)){ #max patients reached
      ...
      if(length(allowable_final)>0){ #if there are doses admissiable in the set
        ...
        dose_rec<-allowable_final[which(UTILITY[allowable_final]==max(UTILITY[allowable_final]))]
