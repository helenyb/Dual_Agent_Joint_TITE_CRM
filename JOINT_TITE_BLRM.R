...
gibbs_sampler.blrm.combo<-function(data_list,iter,model_string){
  ...
  tt<-jags.samples(jags,c('alphaTZ','betaTZ','alphaEZ','betaEZ','alphaTW','betaTW','alphaEW','betaEW','etaT','etaE','phi'),iter,progress.bar="none")
  return(tt)
}
...
#mu_phi=prior_vecI[1] prior mean for phi (Gumbel model)
#tau_phi=prior_vecI[2] prior precision for phi (Gumbel model)
#mu_eta_T=prior_vecI[3] prior mean for eta_T (W/Z interaction for toxicity)
#tau_eta_T=prior_vecI[4] prior precision for eta_T (W/Z interaction for toxicity)
#mu_eta_E=prior_vecI[5] prior mean for eta_E (W/Z interaction for efficacy)
#tau_eta_E=prior_vecI[6] prior precision for eta_E (W/Z interaction for efficacy)
...
JOINT.TITE.BLRM<-function(seed,tru.E.pars,tru.T.pars,tru.corET,co_size,ncohorts ,target,
  ...
  while(stop==0){
    ...
    if(initial==0){
      ...
      currentdata_blrm<-list(ndosesZ=ndosesZ, ndosesW=ndosesW,
                             weightE=weightE,weightT=weightT,
                             yOUT=category_function(T_vec=y_T, E_vec=y_E),
                             dosesW=dosesW,
                             dosesZ=dosesZ,
                             patdoses=patdosesmat,
                             mu_alphaT_Z=prior_vecZ[1],mu_betaT_Z=prior_vecZ[3],
                             tau_alphaT_Z=prior_vecZ[2],tau_betaT_Z=prior_vecZ[4],
                             mu_alphaE_Z=prior_vecZ[5],mu_betaE_Z=prior_vecZ[7],
                             tau_alphaE_Z=prior_vecZ[6],tau_betaE_Z=prior_vecZ[8],
                             mu_alphaT_W=prior_vecW[1],mu_betaT_W=prior_vecW[3],
                             tau_alphaT_W=prior_vecW[2],tau_betaT_W=prior_vecW[4],
                             mu_alphaE_W=prior_vecW[5],mu_betaE_W=prior_vecW[7],
                             tau_alphaE_W=prior_vecW[6],tau_betaE_W=prior_vecW[8],
                             mu_phi=prior_vecI[1] ,
                             tau_phi=prior_vecI[2] ,
                             mu_etaT=prior_vecI[3] ,
                             tau_etaT=prior_vecI[4] ,
                             mu_etaE=prior_vecI[5] ,
                             tau_etaE=prior_vecI[6])
      #browser()
      ...
      for(z in 1:ndosesZ){
        ...
        piEZ[z] = expit(mean(gibbs_out_blrm$alphaEZ) + mean(gibbs_out_blrm$betaEZ)*dosesZ[z])
      }
      ...
      for(w in 1:ndosesW){
        ...
        piEW[w] = expit(mean(gibbs_out_blrm$alphaEW) + mean(gibbs_out_blrm$betaEW)*dosesW[w])
      ...
      for(z in 1:ndosesZ){
        for(w in 1:ndosesW){
          ...
          oddsE0[w,z] = piEZ[z]/(1-piEZ[z]) + piEW[w]/(1-piEW[w]) + (piEZ[z]*piEW[w])/((1-piEW[w])*(1-piEZ[z]))
          oddsE[w,z] = oddsE0[w,z]*exp(mean(gibbs_out_blrm$etaE)*dosesW[w]*dosesZ[z])
      ...
      for(z in 1:ndosesZ){
        ...
        piEZ_i[z,] = expit((gibbs_out_blrm$alphaEZ) + (gibbs_out_blrm$betaEZ)*dosesZ[z])
      }
      ...
      for(w in 1:ndosesW){
        ...
        piEW_i[w,] = expit((gibbs_out_blrm$alphaEW) + (gibbs_out_blrm$betaEW)*dosesW[w])
      ...
      for(z in 1:ndosesZ){
        for(w in 1:ndosesW){
          for(iter.i in 1:gs.iter){
            ...
            oddsE0_i[w,z,iter.i] = piEZ_i[z,iter.i]/(1-piEZ_i[z,iter.i]) + piEW_i[w,iter.i]/(1-piEW_i[w,iter.i]) + (piEZ_i[z,iter.i]*piEW_i[w,iter.i])/((1-piEW_i[w,iter.i])*(1-piEZ_i[z,iter.i]))
            oddsE_i[w,z,iter.i] = oddsE0_i[w,z,iter.i]*exp((gibbs_out_blrm$etaE[iter.i])*dosesW[w]*dosesZ[z])
            
            
            piT_i[w,z,iter.i] = (oddsT_i[w,z,iter.i])/(1+oddsT_i[w,z,iter.i])
            piE_i[w,z,iter.i] = (oddsE_i[w,z,iter.i])/(1+oddsE_i[w,z,iter.i])
          }
        }
      }
      ...
      for(dose.level in 1:ndoses){
        ...
        probeff[dose.level]<-mean(piE_i[dose.index[1],dose.index[2],]>effbound,na.rm = T)
      ...
      for(dose.level in admiss.doses){
        ...
        p.eff.est[dose.level]<-piE[dose.index[1],dose.index[2]]
      ...
      if(length(nextdose)==0){
        ...
        break
          ...
          }#not past a.stop.bound
        }#if not admissible after pause
      }#if not admissible at first analysis
      ...
      if(((safety.stopping.low.unsafe==T)&(npats_doses[1]>0))|((safety.stopping.high.toosafe==T)&(npats_doses[ndoses]>0))){
        ...
        currentdata_blrm1<-list(ndosesZ=ndosesZ, ndosesW=ndosesW,
                                weightE=weightE1,weightT=weightT1,
                                yOUT=yOUT1,
                                dosesW=dosesW,
                                dosesZ=dosesZ,
        ...
        for(z in 1:ndosesZ){
          ...
          piEZ1_i[z,] = expit((gibbs_out_blrm1$alphaEZ) + (gibbs_out_blrm1$betaEZ)*dosesZ[z])
        }
        ...
        for(w in 1:ndosesW){
          ...
          piEW1_i[w,] = expit((gibbs_out_blrm1$alphaEW) + (gibbs_out_blrm1$betaEW)*dosesW[w])
        ...
        for(z in 1:ndosesZ){
          for(w in 1:ndosesW){
            for(iter.i in 1:gs.iter){
              ...
              oddsE0_i[w,z,iter.i] = piEZ_i[z,iter.i]/(1-piEZ1_i[z,iter.i]) + piEW1_i[w,iter.i]/(1-piEW1_i[w,iter.i]) + (piEZ1_i[z,iter.i]*piEW1_i[w,iter.i])/((1-piEW1_i[w,iter.i])*(1-piEZ1_i[z,iter.i]))
              oddsE_i[w,z,iter.i] = oddsE01_i[w,z,iter.i]*exp((gibbs_out_blrm1$etaE[iter.i])*dosesW[w]*dosesZ[z])
